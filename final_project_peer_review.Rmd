...
There are a couple of issues to be solved before we can do further analysis:  
...
One function (tidy_for_v) is designed to  
...
Peer review note: Looks great, nice job! Here where you descibe the two functions I wonder if you might want to break it up into two seperate chuncks? I see the benefit of keeping them together as function #2 uses function #1 but in some ways it seems slightly easier to follow if you break it up into two pieces and move the text desribing function two. It just seems easier to get a grasp on the two functions that way, but just a thought, looks great either way. 
...
A second function (tidy_for_subject) is designed to apply aforementioned function to all meaningless variables (V2-V31). It takes the data of 1 subject as input.
...
Phase portrait is designed to plot the normalized angular positions $\Theta$ (x-axis) and the normalized angular velocities $\omega_{i}$ (y-axis). Angular positions and velocities were normalized using the following equations, where $\Theta_{i}$ and $\omega_{i}$ are the angular position and velocity for 101 data points during a walking cycle.  
...
Peer review note: I wondered if you might want to add just a little bit more about the phase angle, the crp, and the dp before each chunk? I still felt slightly unsure what these were, and how they were useful. Maybe that is beyond the scope of what you want/need to include for this project but slightly more text, even another sentence, explaing each step might have made it slightly easier for me to get a grasp on the usefulness of each piece, but you've clearly put a lot of time and thought into this already and it looks great, and on the other hand more text might end up added clutter. Looks great. 
...
Peer review note: This looks really great. I changed or added a word here or there, and added a few notes mainly suggesting the possibility of considering adding one ore two additional sentances on the steps used to caclulate dp, crp, and phase angle. This looks really great to me, and it's clear that each step feeds into the next to complete a cohesive product. The introduction aids in outlining an overall goal and tying the pieces together. This is really effective as an overall product with a clear purpose and I think it is well thought out. The code seems compact and to the point I honestly don't have a lot of suggestions, and I think you've made an impressive product. Nice job! 
\
